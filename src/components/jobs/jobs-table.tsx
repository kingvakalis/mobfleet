...
function Th({ children, className }: { children?: React.ReactNode; className?: string }) {
  return (
    <th scope="col" className={`label px-4 py-2.5 font-normal text-fg-muted ${className ?? ''}`}>{children}</th>
  )
}
    ...
    <tr className="border-b border-line transition-colors hover:bg-panel">
      ...
      <td className="px-4 py-2.5 text-right">
        {job.status === 'failed' && canRetry && (
          <button
            type="button"
            onClick={() => { safe(client.retryJob(job.id), 'Could not retry job'); logAudit({ actor: employee.name, action: 'job.retried', target: TASK_LABEL[job.type] ?? job.type, detail: job.id, result: 'success' }) }}
