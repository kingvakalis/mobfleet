...
import { CORE_READ_COLUMNS, readColumnDriftAgainstDmmf, teamRelations } from './relations'
...
// ── Read-column inventory (Phase 3B column-drift tolerance) ──
test('CORE_READ_COLUMNS only names scalar fields that exist in the Prisma DMMF (no stale columns)', () => {
  // If a core model field is renamed/removed in the schema, this surfaces here instead of the
  // inventory silently selecting a column that no longer exists.
  assert.deepEqual(readColumnDriftAgainstDmmf(), [])
})

test('CORE_READ_COLUMNS excludes the Phase 3A columns (those are reported by the 3A presence check)', () => {
  const team = CORE_READ_COLUMNS.Team.map((c) => c.column)
  assert.ok(!team.includes('supabaseTeamId') && !team.includes('archivedAt'), 'Team 3A columns are not in the legacy read set')
  assert.ok(!CORE_READ_COLUMNS.Invite.some((c) => c.column === 'invitedByUserId'), 'Invite 3A column is not in the read set')
  // The production-failing column IS inventoried, tagged as a parity input.
  const overrides = CORE_READ_COLUMNS.Membership.find((c) => c.column === 'overrides')
  assert.ok(overrides && overrides.impacts.includes('parity'), 'Membership.overrides is inventoried as a parity column')
})

test('every read column declares at least one impact (count|identity|artifact|parity)', () => {
  const VALID = new Set(['count', 'identity', 'artifact', 'parity'])
  for (const cols of Object.values(CORE_READ_COLUMNS)) for (const c of cols) {
    assert.ok(c.impacts.length > 0, `${c.column} has impacts`)
    for (const i of c.impacts) assert.ok(VALID.has(i), `${c.column} impact ${i} is valid`)
  }
})
